...
%%%%%%%%
\chapter{Case: Defining New Contracting Processes}\label{c:case:consulting-contracts}
...
\begin{enumerate}
	\item Referring physician, previously approved by and registered on Market-X, accesses Market-X software.
	\item If the Referring physician has not yet added her Patient’s information and contact details, she adds them. 
	\item Referring physician searches Market-X and finds relevant Supply side resources, which she decides to book for her Patient. Market-X uses Patient information and search input from the Referring physician to recommend best fitting resources in search results to the Referring physician.
	\item Referring physician makes a tentative booking of the resources. Market-X starts countdown until payment and booking confirmation. If countdown completes before Patient makes the payment, resources are freed up and available to others.
	Market-X notifies Patient that Referring physician has made a tentative booking, and that she needs to accept or refuse that booking. 
	\item Cases:
		\begin{enumerate}
			\item If Patient accepts, Patient makes the payment on X. Market-X notifies the Referring physician and the booked resource of this booking and shares the relevant Patient information with the booked resource.
		...
		\end{enumerate}
	\item Patient uses the booked resource, meaning the booked consultation or otherwise takes place. Market-X allows the booked resource to add results of the consultation. Market-X notifies the Referring physician and the Patient that they can access the results on X.
	\item Market-X asks Referring patient to give feedback and score the booked resource. Market-X asks Referring physician to rank give feedback on the results which the booked resource posted for the Patient on X. X uses the evaluations to describe the quality or reputation of the booked resource.
\end{enumerate}

\section{\ncnf{} for Market-X}
The initial set of terms was the following.

\newterm{Patient: An individual who needs an Appointment, and whose Appointment is booked via Market-X. There are two types of Patients:
\begin{itemize}
	\item Referred Patient: Patient who has a Referring Physician. The Referring Physician books Appointments on behalf of the Patient on Market-X.
	\item Self-Service Patient: Patient who does not have a Referring Physician outside Market-X. Patient needs to book Appointments by herself on Market-X.
\end{itemize}
}

\newterm{Physician: Medical professional who holds a valid license to practice medicine. There are the following types of Physician Roles on Market-X:
\begin{itemize}
	\item Referring Physician: Physician who is not part of Market-X (the business), and uses Market-X to refer the Patient to a Referred Physician.
	\item Referred Physician: Physician to whom the Patient is being referred via Market-X, and whom the Patient will have an Appointment with. 
\end{itemize}
}

\newterm{Visitor: Individual who is visiting Market-X and viewing public Market-X material (such as website, mobile app, etc.) and who has not yet registered as a Patient on Market-X.}

\newterm{X Administrators: An individual employed by Market-X, and who is authorized to make changes to User Rights, Appointments, and more generally any subset of data which Market-X holds.}

\newterm{Appointment: A limited amount of time with a set start and end, which a Patient purchased from a Physician on Market-X, and during which the Physician is available to provide diagnostic and prognostic care to the Patient.}
...
\newterm{Market-X Health Record (XHR): the electronic health record of the Patient on Market-X.}


It turned out that many \nterm{Patients} wanted to see \nterm{Physicians} without this being initiated by a referring \nterm{Physician}. In such cases, it was necessary for \nterm{Market-X}, the business, to provide a referring \nterm{Physician}. This also meant refining the definition of \nterm{Physician}, to include so-called \nterm{Market-X Physician}, giving the following new \nterm{Physician} definition.

\newterm{Physician: Medical professional who holds a valid license to practice medicine. There are the following types of Physician Roles on Market-X:
\begin{itemize}
	\item Referring Physician: Physician who is not part of Market-X (the business), and uses Market-X to refer the Patient to a Referred Physician.
	\item Referred Physician: Physician to whom the Patient is being referred via Market-X, and whom the Patient will have an Appointment with. 
	\item Market-X Physician: Physician employed by Market-X, and who acts as a Referring Physician for every Self-Service Patient on Market-X.
\end{itemize}
}
...
\nterm{Physician Staff User: Market-X User who was invited by a Physician to Market-X, and will accomplish tasks (registering Patients, managing Appointment Slots, etc.) on behalf of Physician on X.
}
...
\begin{enumerate}
	\item Physician User logs into Market-X.
	\item Physician User chooses Staff.
	\item Physician User chooses to invite new Physician Staff Users.
	\item Market-X asks the Physician User to provide Physician Staff User Registration Properties.
	\item Physician User fills out the information and clicks to submit it to Market-X.
	\item Market-X does the following:
		\begin{enumerate}
			\item Market-X changes the Physician Staff User’s Market-X Status to Awaiting;
			\item Market-X sends New Physician Staff Password Setup email to the Physician Staff.
		\end{enumerate}
...
Physician Staff clicks on the link in the email.
	\item Market-X opens in Physician Staff User’s web browser, and asks Physician Staff to enter twice a new password, which Physician Staff User wishes to use to access Market-X.
	\item Physician Staff User submits the password.
	\item Market-X checks if the password is strong enough, and if not, requests a new password from Physician Staff User.
	\item If the chosen password is acceptable to Market-X, Market-X performs the following:
		\begin{enumerate}
			\item Market-X registers Physician Staff as a Physician Staff User on Market-X.
			\item Market-X changes Physician Staff User Status to Active.
			\item Market-X gives Physician Staff the User Rights chosen by the Physician who invited the Physician Staff User.
			\item Market-X sends an email to the Physician Staff User, to confirm that she is registered to Market-X.
			\item Market-X sends an email to the Physician User who invited the Physician Staff User, to notify Physician User that Physician Staff User is registered and Active on Market-X.
		\end{enumerate}
\end{enumerate}
}
...
\chapter{Case: Defining a New Product Development Process}
...
\chapter{Case: Defining an Automated Running Coach}
Over two years, from 2011 to 2013, the \ncnf in this case went from a dozen or so terms, to about one hundred. Not only was the language used in verbal and written communication, but also in algorithm specifications, software specifications, software code, and marketing material for the service which this software was enabling. It is a rare case where the language was built for completely new ideas, and graduated to be included in most artefacts used and produced by the company who commissioned the underlying innovation.
